...
const path = arg?.split("=")[1] ?? "./settings.prd.js";
...
if (fs.existsSync(path)) {
    ...
    if (arg) {
        ...
        console.error("Error: the settings.prd.js file does not exist.");
    }
    process.exit(1);
}
...
const clientToUse =
    settings.cliOptions?.clientToUse || settings.clientToUse || "chatgpt";
...
async function conversation() {
    ...
    prompt.ui.activePrompt.opt.source = (answers, input) => {
        ...
        return availableCommands.filter((command) =>
            command.value.startsWith(input)
        );
    };
...
async function resumeConversation() {
    conversationData =
        (await client.conversationsCache.get("lastConversation")) || {};
...
async function copyConversation() {
    if (clientToUse !== "chatgpt") {
        logWarning(
            "Copying conversations is only supported for ChatGPT client."
        );
        return conversation();
    }
    if (!conversationData.conversationId) {
